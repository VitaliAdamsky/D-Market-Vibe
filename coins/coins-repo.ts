...
/**
 * `CoinsRepo` - это статический класс, предназначенный для управления
...
export class CoinsRepo {
  ...
  public static async initialize(): Promise<void> {
    ...
    if (CoinsRepo._initialized) {
      console.log(
        "🔵 CoinsRepo уже инициализирован. Пропускаем повторную инициализацию."
      );
      return;
    }
    ...
    try {
      ...
      CoinsRepo._initialized = true; // Устанавливаем флаг инициализации в true.
      console.log("🔵 CoinsRepo → инициализирован...");
    } catch (error) {
      console.error("❌ Ошибка при инициализации CoinsRepo:", error);
  ...
  public static async getCoinsFromCache(): Promise<{
    ...
    if (!CoinsRepo._initialized) {
      throw new Error(
        "CoinsRepo не инициализирован. Вызовите статический метод initialize() сначала."
      );
    }
